#' predict_lavaan
#'
#' @param fit model from lavaan sem
#' @param newdata new data to use for predictions
#'
#' @return predictions from SEM lavaan model
#' @export
predict_lavaan <- function(fit, newdata = NULL){
  # https://github.com/yrosseel/lavaan/issues/44#ref-issue-124989077
  ...
  if (":=" %in% mod_df$op) mod_df = mod_df[-which(mod_df$op == ":="),]
  ...
  for(r in unique(mod_df$lhs)){
    ...
    mod_mat <- stats::model.matrix(form, newdata)[,-1]
